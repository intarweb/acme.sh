# Fork tracking — intarweb/acme.sh

This is a soft fork of [`acmesh-official/acme.sh`](https://github.com/acmesh-official/acme.sh). We track upstream HEAD and publish a HEAD-tracked image to GHCR with any open intarweb→upstream PRs cherry-picked on top.
...
| Property | Value |
|---|---|
| Upstream | [`acmesh-official/acme.sh`](https://github.com/acmesh-official/acme.sh) |
...
| Property | Value |
|---|---|
| Image | `ghcr.io/intarweb/acme.sh` |
...
services:
  acme.sh:
    image: ghcr.io/intarweb/acme.sh:latest
```

Pin to a specific build for reproducibility:

```yaml
    image: ghcr.io/intarweb/acme.sh:sha-<short>
```
...
```bash
gh workflow run "Sync from upstream + auto-regen intarweb-dev" --repo intarweb/acme.sh
```

**Force a fresh build of `:latest` without an upstream change:**
```bash
gh workflow run "Build from source → GHCR" --repo intarweb/acme.sh --ref intarweb-dev
```
...
gh pr create --repo acmesh-official/acme.sh --base master --head intarweb:feat/my-thing
